...
const navSections = [
  { icon: 'fa-compress-arrows-alt', title: 'Optimizar', desc: 'Reduce hasta 80% el tamaño sin sacrificar calidad. Inteligencia FFmpeg.', color: '--success', link: '/optimizar', emoji: '🚀', tooltip: 'Comprime videos manteniendo calidad HD' },
  { icon: 'fa-palette', title: 'Editar', desc: 'Ajusta brillo, contraste, velocidad, recorta y aplica filtros profesionales.', color: '--Mora', link: '/editar', emoji: '🎨', tooltip: 'Editor profesional con efectos en tiempo real' },
  ...
  { icon: 'fa-globe', title: 'Online', desc: 'Reproduce YouTube, TikTok, Facebook, Twitch sin anuncios, rastreo ni esperas.', color: '--warning', link: '/online', emoji: '🌐', tooltip: 'Visor sin publicidad ni interrupciones' }
];

const stats = [
  { number: 4, label: 'Herramientas Pro', suffix: '+', icon: 'fa-tools' },
  { number: 100, label: 'Privacidad Garantizada', suffix: '%', icon: 'fa-shield-alt' },
  { number: 15, label: 'Formatos Soportados', suffix: '+', icon: 'fa-file-video' },
  { number: 8, label: 'Resolución Máxima', suffix: 'K', icon: 'fa-tv' }
];

const roles = [
  '🎬 Edición Profesional FFmpeg',
  '🚀 Compresión Inteligente hasta 80%',
  '🔄 Conversión Universal Sin Límites',
  '🌐 Reproductor Multiplataforma Privado',
  '✂️ Recorte de Precisión Milimétrica',
  '🎨 Filtros y Efectos de Estudio'
];

const features = [
  { icon: 'fa-server', title: 'Procesamiento 100% Local', desc: 'Privacidad absoluta. Tus videos nunca abandonan tu dispositivo. Cero servidores externos.', gradient: 'linear-gradient(135deg, #00f3ff 0%, #0EBEFF 100%)' },
  { icon: 'fa-bolt', title: 'Velocidad Extrema', desc: 'Motor FFmpeg con aceleración GPU. Procesamiento hasta 5x más rápido que otras herramientas.', gradient: 'linear-gradient(135deg, #ffa726 0%, #ff9800 100%)' },
...
const techs = [
  ...
  { icon: '🚀', name: 'Progressive Web App', desc: 'Instalable como aplicación nativa en cualquier dispositivo' }
];

const testimonials = [
  { text: 'Perfecto para mis videos de TikTok. Comprime sin perder calidad y es súper rápido.', author: 'María G.', role: 'Creadora de Contenido' },
  { text: 'La mejor herramienta gratuita que he probado. Sin marcas de agua ni trucos ocultos.', author: 'Carlos R.', role: 'Editor de Video' },
  { text: 'Uso VideoWii todos los días. La privacidad local es un diferencial increíble.', author: 'Ana L.', role: 'YouTuber' }
];
...
export const render = () => `
  <div class="inicio_container">
    <section class="hero">
      ...
      <div class="hero_particles">${Array.from({length:20},(_, i)=>`<div class="particle" style="--i:${i}"></div>`).join('')}</div>
      
      <div class="hero_content">
        <div class="hero_badge"><i class="fas fa-sparkles"></i><span>${Saludar()}Tigre!</span></div>
        <h1 class="hero_title">Transforma Videos con <span class="gradient_text">${app}</span></h1>
        ...
        <div class="hero_stats">
          ${stats.slice(0,3).map(s=>`<div class="stat_card"><div class="stat_icon"><i class="fas ${s.icon}"></i></div><div class="stat_number" data-target="${s.number}">0</div><div class="stat_label">${s.label}</div></div>`).join('')}
        </div>
      ...
      <div class="hero_visual">
        ...
        ${navSections.map((s,i)=>`<div class="floating_icon icon${i+1}" data-tooltip="${s.tooltip}"><i class="fas ${s.icon}"></i></div>`).join('')}
      </div>
    </section>
    ...
    <section class="features">
      ...
      <div class="features_grid">${features.map((f,i)=>`<div class="feature_card" style="--delay:${i*0.1}s"><div class="feature_icon" style="background:${f.gradient}"><i class="fas ${f.icon}"></i></div><h3>${f.title}</h3><p>${f.desc}</p></div>`).join('')}</div>
    </section>
    ...
    <section class="nav_visual">
      ...
      <div class="nav_grid">${navSections.map((s,i)=>`<a href="${s.link}" class="nav_card" style="--delay:${i*0.1}s"><div class="nav_card_header"><div class="nav_card_icon" style="background:linear-gradient(135deg,var(${s.color}) 0%,var(--hv) 100%)"><i class="fas ${s.icon}"></i></div><div class="nav_card_emoji">${s.emoji}</div></div><h3>${s.title}</h3><p>${s.desc}</p><div class="nav_card_arrow"><span>Explorar</span><i class="fas fa-arrow-right"></i></div></a>`).join('')}</div>
    </section>
    ...
    <section class="testimonials">
      ...
      <div class="testimonials_grid">${testimonials.map((t,i)=>`<div class="testimonial_card" style="--delay:${i*0.1}s"><div class="testimonial_quote"><i class="fas fa-quote-left"></i></div><p class="testimonial_text">"${t.text}"</p><div class="testimonial_author"><strong>${t.author}</strong><span>${t.role}</span></div></div>`).join('')}</div>
    </section>
    ...
    <section class="tech_section">
      ...
      <div class="tech_grid">${techs.map((t,i)=>`<div class="tech_card" style="--delay:${i*0.1}s"><div class="tech_icon">${t.icon}</div><h4>${t.name}</h4><p>${t.desc}</p></div>`).join('')}</div>
    </section>
    ...
    <section class="stats_section">
      ...
      <div class="stats_grid">${stats.map(s=>`<div class="stat_box"><div class="stat_number" data-count="${s.number}">0${s.suffix}</div><div class="stat_label">${s.label}</div></div>`).join('')}</div>
    </section>

    <section class="cta_section">
      <div class="cta_content">
        <h2 class="cta_title">Crea Contenido Profesional en Minutos</h2>
        <p class="cta_subtitle">Únete a miles de creadores que ya optimizan, editan y convierten sus videos con ${app}. Sin instalación, sin registro, sin complicaciones. Solo resultados profesionales al instante.</p>
        ...
        <div class="cta_actions">
          ...
          <a href="/acerca" class="btn_secondary btn_large"><i class="fas fa-info-circle"></i><span>Conocer ${app}</span></a>
        </div>
      </div>
    </section>
  </div>
