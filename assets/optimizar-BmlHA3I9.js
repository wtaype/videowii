import{j as e}from"./vendor-gzd0YkcT.js";import{c as x,N as s,e as w}from"./main-BbL8IZjR.js";import"./main-DuKZlWId.js";const S=typeof window>"u"?"":window.location.hostname==="localhost"?"http://localhost:3000":window.location.origin,F=()=>`
...
`,M=()=>{console.log(`✅ Optimizar de ${x} cargado`);let t=null,r={},d=!1;e("#uploadZone").on("dblclick",()=>{e("#videoInput").click()}),e("#uploadZone").on("dragover",i=>{i.preventDefault(),e(i.currentTarget).addClass("dragover")}),e("#uploadZone").on("dragleave",i=>{e(i.currentTarget).removeClass("dragover")}),e("#uploadZone").on("drop",i=>{i.preventDefault(),e(i.currentTarget).removeClass("dragover");const a=i.originalEvent.dataTransfer.files;a.length>0&&m(a[0])}),e("#videoInput").on("change",i=>{const a=i.target.files[0];a&&m(a)}),e(document).on("click","#btnSelect",()=>{d||e("#videoInput").click()}),e(document).on("click","#btnDelete",()=>{if(d){s("No puedes eliminar mientras se optimiza","warning",2e3);return}confirm("¿Estás seguro de eliminar este video?")&&(f(),s("Video eliminado","success",2e3))}),e(document).on("input","#qualitySlider",function(){e("#qualityValue").text(e(this).val())}),e(document).on("click","#btnOptimize",async()=>{if(console.log("🎯 [Optimize] Botón optimizar clickeado"),!t){console.warn("⚠️ [Optimize] No hay video cargado"),s("No hay video para optimizar","error",2e3);return}if(d){console.warn("⚠️ [Optimize] Ya hay optimización en progreso"),s("Ya hay una optimización en progreso","warning",2e3);return}console.log("✅ [Optimize] Iniciando optimización..."),await y()});async function y(){d=!0,w("#btnOptimize",!0,"Optimizando..."),e("#progressSection").fadeIn(),l(0,"Preparando video...");try{const i=e("#qualitySlider").val(),a=e("#resolutionSelect").val(),o=e("#codecSelect").val();console.log("🎬 Iniciando optimización:",{quality:i,resolution:a,codec:o});const n=new FormData;n.append("video",t),n.append("quality",i),n.append("resolution",a),n.append("codec",o),l(10,"Subiendo video al servidor...");const v=await fetch(`${S}/optimize`,{method:"POST",body:n});if(!v.ok)throw new Error(`Error del servidor: ${v.statusText}`);l(50,"Optimizando video en el servidor...");const p=await v.json();if(console.log("✅ Respuesta del servidor:",p),!p.success)throw new Error(p.error||"Error desconocido");l(90,"Descargando video optimizado...");const V=`${S}${p.downloadUrl}`,g=await(await fetch(V)).blob(),h=t.size,z=g.size,_=((1-z/h)*100).toFixed(1);l(100,"Completado!"),console.log(`✅ Optimización completa: ${_}% reducción`);const u=document.createElement("a"),b=URL.createObjectURL(g);u.href=b,u.download=`optimizado_${t.name}`,u.click(),URL.revokeObjectURL(b),setTimeout(()=>{e("#progressSection").fadeOut(),s(`✅ Video optimizado! Reducción: ${_}% (${c(h)} → ${c(z)})`,"success",5e3)},1e3)}catch(i){console.error("❌ Error optimizando:",i),s(`Error al optimizar: ${i.message}`,"error",4e3),e("#progressSection").fadeOut()}finally{d=!1,w("#btnOptimize",!1)}}function l(i,a){e("#progressPercent").text(`${i}%`),e("#progressFill").css("width",`${i}%`),e("#progressMessage").text(a)}function m(i){if(console.log("📁 [Upload] Archivo seleccionado:",i.name,i.type,c(i.size)),!i.type.startsWith("video/")){console.warn("⚠️ [Upload] Tipo de archivo inválido:",i.type),s("Por favor selecciona un archivo de video válido","error",3e3);return}t=i;const a=URL.createObjectURL(i),o=document.getElementById("optimizarVideo");o.src=a,o.onloadedmetadata=()=>{r={duration:o.duration,width:o.videoWidth,height:o.videoHeight,size:i.size,format:i.type.split("/")[1].toUpperCase(),name:i.name},console.log("✅ [Upload] Metadata del video:",r),e("#noVideoPlaceholder").hide(),e("#optimizarVideo").show(),e("#videoDuration").text(O(o.duration)),e("#videoResolution").text(`${o.videoWidth}x${o.videoHeight}`),e("#videoSize").text(c(i.size)),e("#videoFormat").text(r.format),e("#videoBitrate").text(I(i.size,o.duration)),e("#uploadZone").hide(),e("#videoInfoPanel").fadeIn(),s("¡Video cargado exitosamente! 🎬","success",2e3)},o.onerror=n=>{console.error("❌ [Upload] Error cargando video:",n),s("Error al cargar el video. Intenta con otro archivo.","error",3e3),f()}}function f(){const i=document.getElementById("optimizarVideo");i&&(i.onloadedmetadata=null,i.onerror=null,i.src&&(URL.revokeObjectURL(i.src),i.src="",i.load())),e("#optimizarVideo").hide(),e("#noVideoPlaceholder").show(),e("#videoInfoPanel").hide(),e("#uploadZone").show(),e("#videoInput").val(""),e("#progressSection").hide(),t=null,r={}}function O(i){const a=Math.floor(i/60),o=Math.floor(i%60);return`${a}:${o.toString().padStart(2,"0")}`}function c(i){return i<1024?i+" B":i<1024*1024?(i/1024).toFixed(2)+" KB":(i/(1024*1024)).toFixed(2)+" MB"}function I(i,a){const o=i*8/a/1e3;return o>1e3?(o/1e3).toFixed(2)+" Mbps":o.toFixed(0)+" kbps"}},$=()=>{console.log("🧹 Optimizar limpiado"),e("#uploadZone, #videoInput, #btnSelect, #btnDelete, #btnOptimize, #qualitySlider").off();const t=document.getElementById("optimizarVideo");t&&t.src&&URL.revokeObjectURL(t.src)};export{$ as cleanup,M as init,F as render};
